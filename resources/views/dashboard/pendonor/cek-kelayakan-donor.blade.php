...
<div id="verificationModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-full max-w-md shadow-lg rounded-lg bg-white">
        <div class="text-center">
            <div class="mb-4">
                <i class="far fa-clock text-gray-600" style="font-size: 80px;"></i>
            </div>
            <h3 class="text-xl font-bold text-gray-900 mb-3">Menunggu Verifikasi</h3>
            <p class="text-gray-600 mb-6">Terima kasih! Data Anda sedang diverifikasi oleh tim kami.</p>
...
<script>
    const questions = [
        "Apakah Anda pernah mengalami alergi berat atau reaksi serius terhadap obat, makanan, atau transfusi darah sebelumnya?",
        "Apakah Anda sedang hamil, menyusui, atau baru melahirkan dalam 6 bulan terakhir? (khusus perempuan)",
        "Apakah Anda pernah mendapat operasi besar atau menerima transfusi darah dalam 1 tahun terakhir?",
        "Apakah Anda pernah bepergian ke daerah endemik malaria dalam 1 tahun terakhir?",
        "Apakah Anda sedang mengonsumsi obat-obatan tertentu saat ini?",
        "Apakah Anda pernah memiliki atau memiliki riwayat penyakit menular seperti hepatitis, HIV/AIDS, atau sifilis?",
        "Apakah dalam 6 bulan terakhir Anda pernah ditato, ditindik, atau menjalani akupuntur?",
        "Apakah usia Anda antara 17-60 tahun?",
        "Apakah berat badan Anda lebih dari 45 kg?",
        "Apakah Anda sedang dalam kondisi sehat dan tidak sedang demam, batuk, pilek, atau flu?"
    ];
    ...
    function showQuestion() {
        ...
        container.innerHTML = `
            <div class="space-y-6">
                <p class="text-lg font-semibold text-gray-900">${questions[currentQuestion]}</p>
                ...
                <div class="space-y-3">
                    <label class="flex items-center p-4 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-red-300 transition-colors">
                        <input type="radio" name="q${currentQuestion}" value="yes" onchange="handleAnswer('yes')" class="w-4 h-4 text-red-600 focus:ring-red-500">
                    ...
                    <label class="flex items-center p-4 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-red-300 transition-colors">
                        <input type="radio" name="q${currentQuestion}" value="no" onchange="handleAnswer('no')" class="w-4 h-4 text-red-600 focus:ring-red-500">
    ...
    function handleAnswer(answer) {
        ...
        // Check for disqualifying answers (questions 0-6 should be "no", 7-9 should be "yes")
